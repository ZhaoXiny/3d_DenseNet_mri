...
class DenseNet3D(object):
  ...
  def _build_graph(self):
    ...
    TP = tf.count_nonzero(tf.argmax(prediction, 1) * tf.argmax(self.labels, 1))
    TN = tf.count_nonzero((tf.argmax(prediction, 1) - 1) * (tf.argmax(self.labels, 1) - 1))
    FP = tf.count_nonzero(tf.argmax(prediction, 1) * (tf.argmax(self.labels, 1) - 1))
    FN = tf.count_nonzero((tf.argmax(prediction, 1) - 1) * tf.argmax(self.labels, 1))
  ...
  def train_all_epochs(self, train_params):
    ...
    for epoch in range(start_epoch, n_epochs + 1):
      ...
      if train_params.get('validation_set', False):
        print("Validation...")
        loss, acc, recall, precision, f1 = self.test(
          self.data_provider.validation, batch_size)
        if self.should_save_logs:
          self.log_loss_accuracy(loss, acc, epoch, prefix='valid')
          print("mean recll: %f, mean precision: %f, mean f1: %f " % (recall, precision, f1))
  ...
  def test(self, data, batch_size):
    ...
    for i in range(num_examples // batch_size):
      ...
      fetches = [self.cross_entropy, self.accuracy, self.recall, self.precision, self.f1]
      loss, accuracy, recall, precision, f1 = self.sess.run(fetches, feed_dict=feed_dict)
    ...
    return mean_loss, mean_accuracy, mean_recall, mean_precision, mean_f1
